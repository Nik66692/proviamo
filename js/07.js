...
function bindAppEvents(){
  ...
  $('#resetGeometryBtn').onclick=()=>applyLayoutPreset(state.layout.preset);
  ...
  $('#artFile').onchange=async e=>{const file=e.target.files[0];if(!file)return;snapshot();state.art.src=await readFileAsDataURL(file);state.art.name=file.name;loadArt();queueDraftSave();};
  ...
  $('#duplicateCardBtn').onclick=async()=>{const c=deepClone(state);c.id=uid();c.content.name=`${c.content.name} copia`;c.createdAt=c.updatedAt=new Date().toISOString();await storePut(STORE_CARDS,c);refreshLibrary();toast('Duplicato salvato','success');};
  ...
  $('#exportPngBtn').onclick=exportPng;$('#exportJsonBtn').onclick=exportCardJson;$('#exportProjectBtn').onclick=exportProject;$('#exportSheetBtn').onclick=()=>exportSheet().catch(e=>toast(e.message,'error'));
  $('#importCardFile').onchange=e=>e.target.files[0]&&importCardFile(e.target.files[0]).catch(err=>toast(err.message,'error'));
  $('#importProjectFile').onchange=e=>e.target.files[0]&&importProjectFile(e.target.files[0]).catch(err=>toast(err.message,'error'));
